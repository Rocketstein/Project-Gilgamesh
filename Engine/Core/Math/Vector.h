...
constexpr Vector2 operator+(const Vector2& v) noexcept { return v; }

// Taking lhs by value lets the compound operator do the work and gives the
// copy elision the canonical form is built around.
constexpr Vector2 operator+(Vector2 lhs, const Vector2& rhs) noexcept { return lhs += rhs; }
constexpr Vector2 operator-(Vector2 lhs, const Vector2& rhs) noexcept { return lhs -= rhs; }

// Scalar multiply is commutative, so it needs both orderings. The left-hand
// scalar form cannot be a member.
constexpr Vector2 operator*(Vector2 v, float scalar) noexcept { return v *= scalar; }
constexpr Vector2 operator*(float scalar, Vector2 v) noexcept { return v *= scalar; }
constexpr Vector2 operator/(Vector2 v, float scalar) noexcept { return v /= scalar; }
...
constexpr float LengthSquared(const Vector2& v) noexcept { return Dot(v, v); }

// std::sqrt is not constexpr before C++26, so this one stays runtime-only.
inline float Length(const Vector2& v) noexcept { return std::sqrt(LengthSquared(v)); }

// Returns the zero vector for a degenerate input rather than producing NaNs.
inline Vector2 Normalized(const Vector2& v) noexcept
{
...
inline bool NearlyEquals(const Vector2& a, const Vector2& b, float epsilon = 1e-5f) noexcept
{
...
constexpr Vector3 operator+(const Vector3& v) noexcept { return v; }

constexpr Vector3 operator+(Vector3 lhs, const Vector3& rhs) noexcept { return lhs += rhs; }
constexpr Vector3 operator-(Vector3 lhs, const Vector3& rhs) noexcept { return lhs -= rhs; }

constexpr Vector3 operator*(Vector3 v, float scalar) noexcept { return v *= scalar; }
...
constexpr float LengthSquared(const Vector3& v) noexcept { return Dot(v, v); }

inline float Length(const Vector3& v) noexcept { return std::sqrt(LengthSquared(v)); }
...
inline bool NearlyEquals(const Vector3& a, const Vector3& b, float epsilon = 1e-5f) noexcept
{
	return std::fabs(a.x - b.x) <= epsilon
		&& std::fabs(a.y - b.y) <= epsilon
...
constexpr Vector4 operator+(const Vector4& v) noexcept { return v; }

constexpr Vector4 operator+(Vector4 lhs, const Vector4& rhs) noexcept { return lhs += rhs; }
constexpr Vector4 operator-(Vector4 lhs, const Vector4& rhs) noexcept { return lhs -= rhs; }

constexpr Vector4 operator*(Vector4 v, float scalar) noexcept { return v *= scalar; }
...
constexpr float LengthSquared(const Vector4& v) noexcept { return Dot(v, v); }

inline float Length(const Vector4& v) noexcept { return std::sqrt(LengthSquared(v)); }
...
inline bool NearlyEquals(const Vector4& a, const Vector4& b, float epsilon = 1e-5f) noexcept
{
	return std::fabs(a.x - b.x) <= epsilon
		&& std::fabs(a.y - b.y) <= epsilon
		&& std::fabs(a.z - b.z) <= epsilon
